...
public class CashierEditPanel extends JPanel {
    ...
    /** Create the panel. */
    public CashierEditPanel(
            JFrame currentFrame, StoreService storeService, Cashier cashier, Boolean isAdd) {
        ...
        btnSave.addActionListener(
                new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        ...
                        String number = textField_1.getText().trim();
                        String password = new String(passwordField.getPassword());

                        if (isAdd && (name.isEmpty() || number.isEmpty() || password.isEmpty())) {
                            JOptionPane.showMessageDialog(
                                    CashierEditPanel.this,
                                    "Name, ID, and password are required for a new cashier.",
                                    "Missing information",
                        ...
                        currentFrame
                                .getContentPane()
                                .add(new CashierListPanel(currentFrame, storeService));
        ...
        // Never display the stored hash. Leaving the field blank keeps the current password.
        passwordField = new JPasswordField();
        passwordField.setBounds(497, 161, 116, 22);
        add(passwordField);
        passwordField.setColumns(10);

        JLabel lblPasswordHint = new JLabel("(blank = keep current)");
        lblPasswordHint.setBounds(497, 186, 160, 16);
        add(lblPasswordHint);
