...
public class CashierReport extends JPanel {
	 ...
	 * Create the panel.
	 */
	public CashierReport(JFrame currentFrame, StoreService storeService) {
		...
		btnGenerate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				textArea.setText("Cashier report for "
						+ datePicker.getDate().format(DateTimeFormatter.ofPattern("M/d/yyyy")) + "\n\n");
				...
				for (Session s : storeService.getStore().getSessions()) {
					if (s.getEndDateTime().toLocalDate().isEqual(datePicker.getDate())) {
						...
						textArea.append(s.getRegister() + "\t" + s.getCashier().getPerson().getName() + "\t\t"
